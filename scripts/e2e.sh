...
for path in / /circles /events /join; do
...
# The seeded directory must actually list circles.
CIRCLES_JSON="$(c "$BASE/api/circles")"
CIRCLE_COUNT="$(printf '%s' "$CIRCLES_JSON" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{try{const j=JSON.parse(s);console.log(Array.isArray(j)?j.length:(j.circles||[]).length)}catch{console.log(0)}})')"
if [ "${CIRCLE_COUNT:-0}" -ge 5 ]; then ok "api lists $CIRCLE_COUNT circles"; else bad "api lists >=5 circles" ">=5" "$CIRCLE_COUNT"; fi

SLUG="$(printf '%s' "$CIRCLES_JSON" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);const a=Array.isArray(j)?j:(j.circles||[]);const pub=a.find(c=>!c.isPrivate)||a[0];console.log(pub.slug)})')"
is "GET /circles/$SLUG is 200" "200" "$(code "$BASE/circles/$SLUG")"

# Pick a gathering that actually has room. The seed deliberately includes a FULL
# one, and booking it is correctly refused — picking blindly made the gate blame
# the app for behaving properly.
EVENTS_JSON="$(c "$BASE/api/events?circle=$SLUG")"
...
if [ -z "$EVSLUG" ]; then bad "circle has a bookable gathering" "one with placesLeft>0" "none"; fi
...
# ------------------------------------------------------------------- join flow
section "Join a circle"
is "POST join redirects" "302" "$(code -X POST "$BASE/circles/$SLUG/join")"
MEMBERSHIPS="$(c "$BASE/api/me")"
has "membership recorded" "$SLUG" "$MEMBERSHIPS"

# ---------------------------------------------------------------- buy the pass
section "Buy a Trio (3-credit) pass"
PKG="$(c "$BASE/api/circles/$SLUG" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);const p=(j.packages||[]).find(p=>p.credits===3);console.log(p?p.id:"")})')"
if [ -z "$PKG" ]; then bad "circle has a 3-credit package" "a package with credits=3" "none"; fi
is "POST buy redirects" "302" "$(code -X POST "$BASE/circles/$SLUG/passes/$PKG/buy")"
CREDITS="$(c "$BASE/api/me" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);const t=(j.passes||[]).reduce((n,p)=>n+(p.creditsTotal-p.creditsUsed),0);console.log(t)})')"
is "3 credits available after purchase" "3" "$CREDITS"

# ------------------------------------------------ gallery, members, attendees
section "Explore: gallery, members, attendees"
CIRCLE_DETAIL="$(c "$BASE/api/circles/$SLUG")"
PHOTOS="$(printf '%s' "$CIRCLE_DETAIL" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);console.log((j.photos||[]).length)})')"
if [ "${PHOTOS:-0}" -ge 4 ]; then ok "circle has $PHOTOS gallery photos"; else bad "circle gallery has >=4 photos" ">=4" "$PHOTOS"; fi
MEMBERS="$(printf '%s' "$CIRCLE_DETAIL" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);console.log((j.members||[]).length)})')"
if [ "${MEMBERS:-0}" -ge 1 ]; then ok "circle lists $MEMBERS members"; else bad "circle lists members" ">=1" "$MEMBERS"; fi

CIRCLE_HTML="$(c "$BASE/circles/$SLUG")"
has "circle page renders the gallery"  'data-gallery'  "$CIRCLE_HTML"
has "circle page renders members"      'data-members'  "$CIRCLE_HTML"
...
ok "gathering shows $ATT_BEFORE attendees before booking"
EV_HTML="$(c "$BASE/events/$EVSLUG")"
has "gathering page renders attendees" 'data-attendees' "$EV_HTML"
has "gathering page renders a gallery" 'data-gallery'   "$EV_HTML"

# ------------------------------------------------------------------- book flow
section "Book a gathering, spend a credit"
is "GET /events/$EVSLUG is 200" "200" "$(code "$BASE/events/$EVSLUG")"
is "POST book redirects" "302" "$(code -X POST "$BASE/events/$EVSLUG/book")"
CREDITS_AFTER="$(c "$BASE/api/me" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);const t=(j.passes||[]).reduce((n,p)=>n+(p.creditsTotal-p.creditsUsed),0);console.log(t)})')"
is "credits 3 -> 2 after booking" "2" "$CREDITS_AFTER"
...
CREDITS_DOUBLE="$(c "$BASE/api/me" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);console.log((j.passes||[]).reduce((n,p)=>n+(p.creditsTotal-p.creditsUsed),0))})')"
is "second booking does not spend another credit" "2" "$CREDITS_DOUBLE"

# A gathering at capacity must refuse, and must not silently take a credit.
if [ -n "$FULLSLUG" ]; then
  section "A full gathering refuses the booking"
  ok "seed provides a full gathering ($FULLSLUG)"
  FULL_CODE="$(code -X POST "$BASE/events/$FULLSLUG/book")"
  if [ "$FULL_CODE" = "302" ] || [ "$FULL_CODE" = "409" ]; then ok "booking a full gathering is refused, not a 500 ($FULL_CODE)"; else bad "full gathering refused" "302 or 409" "$FULL_CODE"; fi
  CREDITS_FULL="$(c "$BASE/api/me" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);console.log((j.passes||[]).reduce((n,p)=>n+(p.creditsTotal-p.creditsUsed),0))})')"
  is "a refused booking spends no credit" "2" "$CREDITS_FULL"
else
  bad "seed provides a full gathering" "one with placesLeft=0" "none"
fi

# ------------------------------------------------------------- mock checkout
section "Mock checkout is a real, visible step"
CHECKOUT="$(c "$BASE/circles/$SLUG/passes/$PKG/checkout")"
is "checkout page 200" "200" "$(code "$BASE/circles/$SLUG/passes/$PKG/checkout")"
...
# --------------------------------------------------------------- cancel refund
section "Cancel returns the credit"
is "POST cancel redirects" "302" "$(code -X POST "$BASE/account/tickets/$CODE/cancel")"
CREDITS_BACK="$(c "$BASE/api/me" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);console.log((j.passes||[]).reduce((n,p)=>n+(p.creditsTotal-p.creditsUsed),0))})')"
is "credits 2 -> 3 after cancel" "3" "$CREDITS_BACK"

# ------------------------------------------------------------------- host flow
section "Host creates a circle, a pass and a gathering"
...
NEWNAME="E2E Test Circle $(date +%s)"
is "POST create circle" "302" "$(codeh -X POST \
  --data-urlencode "name=$NEWNAME" \
  --data-urlencode "tagline=A circle created by the end-to-end gate" \
...
if [ -z "$NEWSLUG" ]; then bad "new circle appears under hosting" "a slug" "none"; fi

is "POST create package" "302" "$(codeh -X POST \
  --data-urlencode "name=Trio" --data-urlencode "credits=3" --data-urlencode "priceCents=90000" \
  "$BASE/host/circles/$NEWSLUG/packages")"
...
is "POST create gathering" "302" "$(codeh -X POST \
  --data-urlencode "title=E2E Chef's Table" \
  --data-urlencode "summary=A gathering created by the end-to-end gate." \
  --data-urlencode "description=Long-form description for the end-to-end gate gathering." \
  ...
  "$BASE/host/circles/$NEWSLUG/events")"

has "new circle is in the public directory" "$NEWSLUG" "$(c "$BASE/api/circles")"
NEWEV="$(c "$BASE/api/events?circle=$NEWSLUG" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const j=JSON.parse(s);const a=Array.isArray(j)?j:(j.events||[]);console.log(a.length?a[0].slug:"")})')"
if [ -n "$NEWEV" ]; then ok "new gathering is publicly listed ($NEWEV)"; else bad "new gathering publicly listed" "a slug" "none"; fi

# ---------------------------------------------------------- authorisation edge
section "A host cannot manage someone else's circle"
is "member is refused on host's circle" "403" "$(code -X POST --data-urlencode "name=X" --data-urlencode "credits=1" --data-urlencode "priceCents=100" "$BASE/host/circles/$NEWSLUG/packages")"

# --------------------------------------------------------------- bad input 400
section "Bad input is rejected with 400, not a 500"
is "login without email" "400" "$(code -X POST -d "name=No Email" "$BASE/auth/login")"
is "booking an unknown gathering" "404" "$(code -X POST "$BASE/events/not-a-real-gathering/book")"
is "unknown circle 404s" "404" "$(code "$BASE/circles/not-a-real-circle")"
...
# ---------------------------------------------------------------- cleanup
# The host journey creates a real circle and gathering. Leaving them behind
...
if [ -x "$PSQL" ]; then
  PGPASSWORD=postgres "$PSQL" "postgres://postgres:postgres@localhost:5432/loop_dev" -q -c "
    delete from circles where slug like 'e2e-test-circle-%';
